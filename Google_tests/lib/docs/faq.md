...
expected crash won't kill the test program (i.e. the parent_ process). As a
result, any in-memory side effects they incur are observable in their respective
sub-processes, but not in the parent_ process. You can think of them as running
in a parallel universe, more or less.

In particular, if you use mocking and the death test statement invokes some mock
methods, the parent_ process will think the calls have never occurred. Therefore,
...
In particular, death tests don't like having multiple threads in the parent_
...
makes it harder to search for real problems in the parent_'s log. Therefore,
